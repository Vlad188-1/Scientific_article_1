...
class SimpleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, transform=None):
        ...
        self.images = [file for file in list(Path(root_dir).glob("*")) if file.suffix.lower() in [".png", ".jpg", ".jpeg"]]
...
def get_metadata(path_to_folder_weights: str, path_to_test_data: str) -> dict[dict[str, int], str: str]:
    ...
    logger.warning(f"Количество фотографий, которые повторяются и не участвуют в тестировании: {len(names_more_2_times)}")
    ...
    with torch.set_grad_enabled(False):
        for _, batch in enumerate(dataloader):
            ...
            for i, out in enumerate(outputs):
                ...
                res.append([Path(image_names[i]).name, predicted_idx, id2animal[predicted_idx]]) #probs]) #{id2animal[k]:round(v,5) for k,v in enumerate(probs_list)}])
...
def create_pred_table(path_to_test_data: str, res_dict: dict, input_size: int = 256, batch_size: int = 4) -> pd.DataFrame:
    
    table = None
    for cls_dir in tqdm(list(Path(path_to_test_data).glob("*")), colour="green"):
        if cls_dir.stem in list(res_dict["mapping"].keys()):
            pred_table_per_cls = inference(path_to_ims=cls_dir, res_dict=res_dict, input_size=input_size, batch_size=batch_size)
...
def count_metrics(GT_table: pd.DataFrame, Pred_table: pd.DataFrame, res_dict: dict):
    ...
    report_metrics = classification_report(GT_table["class"].values, Pred_table["class"].values, target_names=sorted(Pred_table["class"].unique()),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pt_w", "--path_to_folder_weights", default="weights/Classification/tigers_vs_leopards/resnest101e", type=str, help="Path to folder with .pt and mapping.yaml files")
    parser.add_argument("--pt_data", "--path_to_test_data", default="data/Classification/tigers_vs_leopards/test", type=str, help="Path to folder with test data")
